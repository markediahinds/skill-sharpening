 ...
 */
function isEven(num) {
  if (num % 2 === 0) { // conditionals / conditional operators always evaluate to true or false 
    return true
  } else return false
}
...
function isTruthy(val) {
  if (val) { // evaluating to true or false, conditionals evaluate if it is truthy or falsy 
    return true
  } else return false
}
...
function isFalsy(val) {
  if (!val) { // undefined, null, false, 0, "", NAN
    return true
  } else return false
}
